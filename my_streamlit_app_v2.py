...
def creating_connection():
     sqlEngin = create_engine('mysql+pymysql://Victor:victor123;@127.0.0.1')
...
@st.cache
def tidy_all_data():
    # Used to check how much time it takes
    start = time.time()
    ...
    print("After GEOSHAPE", time.time() - start)
    tidyed_trafic_df = tidying_trafic_data_func(trafic_staus, geo_shape_data)
    print("After TRAFIC", time.time() - start)
    
    # transport
    tidyed_transport_df = tidying_transport_data_func(transport_status)
    print("After TRANSPORT", time.time() - start)
    
    return tidyed_vcub_df, tidyed_trafic_df, tidyed_transport_df


# In[82]:
# 3- call the filtering function depending on the asked argument:

def calling_filtering_func(show_all=True , vcub_all_data=False , trafic_all_data=False, transport_all_data=False, asked_day=None, asked_hour=start_time):
    tab_to_display_in_map = {}
    
    tidyed_vcub_df, tidyed_trafic_df, tidyed_transport_df = tidy_all_data()
...
def maping_func(dataframe):
    ...
    folium.Circle(radius = 500, location = ([get_lat_lon()[0],get_lat_lon()[1]])).add_to(final_map)
